...
// AWA-05 — backend: FeesController.SendReminders (already on main). The reminder
// window (days-before-due) is a server-side config value (FeeReminder:DaysBeforeDue),
// not a caller-supplied parameter — this just triggers a pass.
export interface SendFeeRemindersResponse {
  remindersSent: number
}

export function sendPaymentReminders() {
  return request<SendFeeRemindersResponse>('/fees/send-reminders', { method: 'POST' })
}
