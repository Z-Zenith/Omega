...
# --- Unit Tests for anomaly engine ---

def test_no_events_returns_no_flags():
...
def test_jittered_human_typing_not_flagged():
...
def test_uniform_timing_alone_does_not_cross_default_threshold():
...
def test_uniform_timing_plus_paste_ratio_flags():
...
def test_large_paste_burst_combined_with_context_switching_flags():
...
def test_windows_are_scored_independently_per_student_session_assignment():
...
def test_different_assignment_windows_for_same_student_scored_separately():
    window_a = [_event(student_id="s1", assignment_id="a1", event_type="window_blur", offset_seconds=i)
                for i in range(4)]
    window_a.append(_event(student_id="s1", assignment_id="a1", event_type="paste",
                            offset_seconds=5, metadata={"char_count": 500}))
    window_b = [_event(student_id="s1", assignment_id="a2", offset_seconds=o)
                for o in [0, 0.4, 1.1, 1.3, 2.6]]
...
def test_api_suspicious_behaviour_success():
    events = [
        {
            "student_id": "s1",
...
def test_api_suspicious_behaviour_no_flags_for_normal_usage():
...
def test_api_validation_error_empty_student_id():
    payload = {
        "events": [
            {
                "student_id": "",
...
def test_api_validation_error_missing_recorded_at():
    payload = {
        "events": [
            {"student_id": "s1", "event_type": "keystroke"}
        ]
    }
    response = client.post("/api/v1/suspicious-behaviour", json=payload)
    assert response.status_code == 422
